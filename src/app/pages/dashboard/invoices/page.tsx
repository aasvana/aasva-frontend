...
import { useInvoices, useDeleteInvoice } from "@/modules/invoice";
import { SavedInvoice } from "@/modules/invoice";
...
const toRow = (record: SavedInvoice): InvoiceRow => {
  const totals = computeTotals(record.data);
  return {
    id: record.id,
    invoiceNo: record.data.invoiceNo || record.id.slice(0, 8),
...
export default function Invoices() {
  ...
  const rows: InvoiceRow[] = invoices.map(toRow);

  const sortByKey = (data: InvoiceRow[], key: SortKey, order: SortOrder) => {
  ...
  const filterAndSortData = () => {
    const filtered = rows.filter((item) =>
      String(item[sortKey as keyof InvoiceRow])
        .toLowerCase()
  ...
  const handleDelete = (id: string) => {
    ...
    deleteMutation.mutate(id, {
      ...
      onError: () => toast.error("Failed to delete the invoice."),
    });
  };

  const handleDownload = (record: InvoiceRow) => {
    const source = invoices.find((v) => v.id === record.id);
    if (!source) return;
    downloadInvoicePdf(source.data).catch(() =>
                          ...
                          <td className="px-6 py-2.5 whitespace-nowrap text-sm">
                            <span
                              className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${
                                statusClasses[data.status] ?? "bg-gray-100 text-gray-700"
                              }`}
                            >
                              {INVOICE_STATUS_LABELS[data.status as keyof typeof INVOICE_STATUS_LABELS] ??
                                data.status}
                            </span>
                          </td>
                          <td className="px-6 py-2.5 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200">
                          ...
                          <td className="px-6 py-2.5 whitespace-nowrap text-end text-sm font-medium">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent side="left" align="start">
                                ...
                                <DropdownMenuItem
                                  onClick={() =>
                                    router.push(`/dashboard/invoices/${data.id}/view`)
                                  }
                                >
                                  <Eye /> View
                                </DropdownMenuItem>
