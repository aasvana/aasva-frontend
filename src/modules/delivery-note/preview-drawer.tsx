...
import { downloadDeliveryNotePdf } from "./download";
import { DeliveryNoteFormData } from "./schema";
import { DeliveryNotePreview } from "./preview";
import {
  Button,
  Drawer,
  DrawerClose,
...
type DeliveryNotePreviewDrawerProps = {
  mode?: "create" | "edit";
  existingId?: string;
  data?: DeliveryNoteFormData | null;
};
...
export function DeliveryNotePreviewDrawer({
  ...
  const previewOpen = useDeliveryNoteDraftStore((s) => s.previewOpen);
  const closePreview = useDeliveryNoteDraftStore((s) => s.closePreview);
  const storeDraft = useDeliveryNoteDraftStore((s) => s.draft);
  ...
  const handleDownload = async () => {
    ...
    try {
      setDownloading(true);
      await downloadDeliveryNotePdf(draft);
  ...
  const handleSave = () => {
    ...
    const onError = () => {
      toast.error("Failed to save the delivery note.");
    };
    if (mode === "edit") {
        ...
        <div className="flex flex-col h-full">
          <DrawerHeader className="flex flex-row items-start justify-between gap-4 border-b">
            <div className="flex flex-col gap-1">
              <DrawerTitle>Preview Delivery Note</DrawerTitle>
              <DrawerDescription>
                This is how the delivery note will look with the current data.
              </DrawerDescription>
            </div>
            <DrawerClose asChild>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 md:px-10 md:py-8">
            {draft ? (
              <div className="rounded-lg border bg-white shadow-sm">
                <DeliveryNotePreview data={draft} />
              </div>
            ) : (
              <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
                Fill in the delivery note details to see the preview.
              </div>
            )}
          </div>
          ...
          <DrawerFooter className="flex-row justify-end border-t">
            ...
            <Button disabled={!draft || isPending} onClick={handleSave}>
              ...
              {isPending ? "Saving..." : "Save Delivery Note"}
            </Button>
          </DrawerFooter>
        </div>
      </DrawerContent>
